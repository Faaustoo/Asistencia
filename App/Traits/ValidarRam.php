...
trait ValidarRAM {
    ...
    public static function validarDatosRam($data) {
        ...
        if (!isset($data['porcentaje_promocion'])) {
            $errores[] = "El porcentaje de promocion es obligatorio.";
        } elseif (!preg_match($numeros, $data['porcentaje_promocion']) || $data['porcentaje_promocion'] < 0 || $data['porcentaje_promocion'] > 100) {
            $errores[] = "El porcentaje de promocion debe ser un numero entre 0 y 100.";
        }

        if (!isset($data['porcentaje_regular'])) {
            ...
            $errores[] = "El porcentaje regular debe ser un numero entre 0 y 100.";
        }

        if (!isset($data['nota_promocion'])) {
            $errores[] = "La nota de promocion es obligatoria.";
        } elseif (!preg_match($numeros, $data['nota_promocion']) || $data['nota_promocion'] < 0 || $data['nota_promocion'] > 10) {
            $errores[] = "La nota de promocion debe ser un numero entre 0 y 10.";
        }

        if (!isset($data['nota_regular'])) {
            ...
            $errores[] = "La nota regular debe ser un numero entre 0 y 10.";
        }
